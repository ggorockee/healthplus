-- 데이터베이스 초기화 스크립트
-- 경고: 이 스크립트는 모든 관련 테이블의 데이터를 영구적으로 삭제합니다.

-- 기존 테이블 및 함수 삭제 (의존성 고려하여 CASCADE 사용)
...
DROP FUNCTION IF EXISTS get_monthly_statistics(UUID, INTEGER, INTEGER) CASCADE;

-- HealthPlus 약물 관리 앱 데이터베이스 스키마
-- Supabase Project ID: yjkfjytsfnpkahuiajjv
