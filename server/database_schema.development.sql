-- HealthPlus 약물 관리 앱 데이터베이스 스키마 (개발 환경)
-- 개발용 Supabase Project ID: yjkfjytsfnpkahuiajjv
-- 이 파일은 개발 환경에서 사용되는 스키마입니다.
...
-- 개발용 테스트 데이터 테이블 (개발 환경에서만 사용)
CREATE TABLE dev_test_data (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
...
CREATE INDEX idx_dev_test_data_test_name ON dev_test_data(test_name);
...
-- 개발 환경에서는 dev_test_data 테이블에 RLS 비활성화
ALTER TABLE dev_test_data ENABLE ROW LEVEL SECURITY;
...
-- 개발용 테스트 데이터 RLS 정책 (개발 환경에서는 모든 사용자가 접근 가능)
CREATE POLICY "Dev test data access" ON dev_test_data
    FOR ALL USING (true);
...
-- 개발용 샘플 데이터 삽입 (개발 환경에서만 실행)
INSERT INTO dev_test_data (test_name, test_data) VALUES 
('development_setup', '{"environment": "development", "setup_date": "2024-01-01"}'),
('test_user_data', '{"test_users": 5, "sample_medications": 10}');
